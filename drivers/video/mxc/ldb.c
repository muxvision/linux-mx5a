		     ...
		     struct fb_info *fbi)
{
	...
	serial_clk = ldb->spl_mode ? chan.vm.pixelclock * 7 / 2 :
        ...
        printk("imx ldb clock set to %dhz",serial_clk);
	...
	ldb_disable(mddh, fbi);
        printk("clk_get_rate(ldb_di_parent) = %d",clk_get_rate(ldb_di_parent));
        printk("clk_get_rate(ldb_di_sel) = %d",clk_get_rate(ldb_di_sel));
        printk("clk_get_rate(ldb_di_sel_parent) = %d",clk_get_rate(ldb_di_sel_parent));
	return ret;
}
