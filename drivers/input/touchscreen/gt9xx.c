...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    GTP_GPIO_OUTPUT(gtp_int_gpio, 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
...
s32 gtp_read_version(struct i2c_client *client, u16* version)
{
    ...
    if (buf[5] == 0x00)
    {
        GTP_DEBUG("IC Version: %c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[7], buf[6]);
    }
    else
    {
        GTP_DEBUG("IC Version: %c%c%c%c_%02x%02x", buf[2], buf[3], buf[4], buf[5], buf[7], buf[6]);
    }
    return ret;
}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "GTP touchscreen, Address: 0x%02x probed", client->addr);
