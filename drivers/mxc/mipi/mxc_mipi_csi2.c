...
/*!
 * This function is called to set mipi lanes and clk reg value.
 * used by adv7282m and ov5640_mipi.
 * @param   info    mipi csi2 hander
 * @return  Return 0 on succes  or negative error code on fail
 */
int mipi_csi2_sensor_config(struct mipi_csi2_info *info,
        unsigned int lanes, int clk)
{
    if(!info)
        return -1;
    info->lanes = lanes;
...
EXPORT_SYMBOL(mipi_csi2_sensor_config);
...
int mipi_csi2_reset(struct mipi_csi2_info *info)
{
    ...
    if(info->clk_reg)
        mipi_csi2_write(info, info->clk_reg, MIPI_CSI2_PHY_TST_CTRL1);
    else
        mipi_csi2_write(info, 0x00000014, MIPI_CSI2_PHY_TST_CTRL1);

	mipi_csi2_write(info, 0x0000000C, MIPI_CSI2_PHY_TST_CTRL1);
	mipi_csi2_write(info, 0x00000002, MIPI_CSI2_PHY_TST_CTRL0);
	mipi_csi2_write(info, 0x00000000, MIPI_CSI2_PHY_TST_CTRL0);
